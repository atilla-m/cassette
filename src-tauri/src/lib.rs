...
fn send_linux_playback_notification(
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        Err(
            "Desktop notifications are available only on Linux in Cassette 0.1.0-beta.3."
                .to_owned(),
        )
    }
}
...
async fn detect_dvd() -> Result<DvdDetectResult, String> {
    ...
    #[cfg(not(target_os = "linux"))]
    Err("DVD detection and import are available only on Linux in Cassette 0.1.0-beta.3.".to_owned())
}
...
async fn scan_dvd_titles(source: String) -> Result<DvdTitleScanResult, String> {
    ...
    #[cfg(not(target_os = "linux"))]
    {
        let _ = source;
        Err(
            "DVD detection and import are available only on Linux in Cassette 0.1.0-beta.3."
                .to_owned(),
        )
    }
}
...
async fn detect_audio_cd() -> Result<CdDetectResult, String> {
    ...
    #[cfg(not(target_os = "linux"))]
    Err(
        "Audio CD detection and ripping are available only on Linux in Cassette 0.1.0-beta.3."
            .to_owned(),
    )
}
...
async fn lookup_cd_metadata() -> Result<CdMetadataLookupResult, String> {
    ...
    #[cfg(not(target_os = "linux"))]
    Err("Audio CD metadata lookup is available only on Linux in Cassette 0.1.0-beta.3.".to_owned())
}
...
impl VideoPlaybackState {
    fn play(
        ...
        Err(
            "Cassette-controlled video playback is available only on Linux in Cassette 0.1.0-beta.3."
                .to_owned(),
        )
    }
...
fn ffprobe_video_codec_info(_path: &Path) -> VideoCodecInfo {
    VideoCodecInfo {
        ...
        duration_seconds: None,
        error: Some(
            "Video inspection is available only on Linux in Cassette 0.1.0-beta.3.".to_owned(),
        ),
    }
}
...
async fn search_lrclib_lyrics_results(track: &Track) -> Result<Vec<LrclibLyricsResult>, String> {
    let client = reqwest::Client::builder()
        ...
        .user_agent("Cassette/0.1.0-beta.3 local lyrics lookup")
        .build()
